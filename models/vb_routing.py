...
@time: 2021/11/26
@file: vb_routing.py
@describe: This file aims to design Variational-Capsule-Routing.
"""
import torch
import numpy as np
...
class VariationalBayesRouting2d(nn.Module):
    ...
    def forward(self, a_i, V_ji):
        ...
        H_q_j = .5 * self.D * torch.log(
            torch.tensor(2 * np.pi * np.e)) - .5 * self.Elnlambda_j  # posterior entropy H[q*(mu_j, sigma_j)]
    ...
    def update_qparam(self, a_i, V_ji, R_ij):
        ...
        if self.cov == 'diag':
            ...
            self.invPsi_j = self.Psi0 + sigma_j + (self.R_j / self.kappa_j) * (mu_j).pow(
                2)  # priors removed for faster computation
